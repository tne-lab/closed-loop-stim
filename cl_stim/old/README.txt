NOTE 07-22-2019: This documents the closed-loop stimulation system used before early 2019, in "main.vi", which did the following things differently from "CLStimSimple.vi":

 * Output digital events to mark real and sham stims along with the analog stim itself, instead of just analog stim on 2 different channels (1 for real and 1 for sham).
 * If enabled, tried to equalize the latency between crossing event timestamps and the actual stimulation time, to compensate for variability in the time it took to transfer data from the headstage. We eventually realized that adding some latency on average had a worse effect on phase locking than allowing the latency to be more variable but smaller on average.
 * Did not timeout before and after a real stimulation - instead, relied on the Crossing Detector "timeout" functionality for that. We moved it to the stim code because it allows more flexibility in handling real and sham stimulations differently - this code knows which crossings will be real stims and which sham, unlike the CD.
